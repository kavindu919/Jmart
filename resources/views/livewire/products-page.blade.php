            ...
            <div class="flex flex-wrap mb-24 -mx-3">
                <div class="w-full pr-2 lg:w-1/4 lg:block">
                            ...
                            @foreach ($categories as $category)
                                <li class="mb-4" wire:key="{{ $category->id }}">
                                    <label for="{{ $category->slug }}" class="flex items-center dark:text-gray-400 ">
                                        <input id="{{ $category->slug }}" value="{{ $category->id }}" type="checkbox"
                                            wire:model.live="selected_categories" class="w-4 h-4 mr-2">
                            ...
                            @foreach ($brands as $brand)
                                <li class="mb-4" wire:key="{{ $brand->id }}">
                                    <label for="{{ $brand->slug }}" class="flex items-center dark:text-gray-300">
                                        <input wire:model.live="selected_brands" type="checkbox"
                        ...
                        <ul>
                            ...
                            <li class="mb-4">
                                <label for="on_sale" class="flex items-center dark:text-gray-300">
                                    <input id="on_sale" type="checkbox" class="w-4 h-4 mr-2" value="1"
                    ...
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                        ...
                        <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
                        <div>
                            <div class="font-semibold text-blue-400">{{ Number::currency($price_range, 'LKR') }}</div>
                            <input type="range" wire:model.live='price_range'
                                class="w-full h-1 mb-4 bg-blue-100 rounded appearance-none cursor-pointer"
                                max="500000" value="100000" step="100000">
                            <div class="flex justify-between ">
                                <span
                                    class="inline-block text-lg font-bold text-blue-400 ">{{ Number::currency(1000, 'LKR') }}</span>
                                <span
                                    class="inline-block text-lg font-bold text-blue-400 ">{{ Number::currency(500000, 'LKR') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
